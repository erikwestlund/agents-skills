---
name: prose-style
description: Erik's writing voice for Better Running Shoes public prose (trust page, buying guide, articles). Load before drafting or editing any reader-facing prose, and check every draft against the DO NOT list — Erik catalogued these from real drafts and they are firm.
---

# Erik's Prose Style — Better Running Shoes

Erik drafts in `~/Docs/Better Shoes/prose/`. `0-numbers.md` is the only
permitted source for numbers: if a number is not in it, it does not go on the
page — ask Erik to verify it instead.
...
- Round large corpus/inventory numbers: "hundreds of thousands of outside
  ...
  "386."
- Source of truth: `~/Docs/Better Shoes/prose/0-numbers.md`. Regenerated
  values always win over anything hand-carried.
...
- **External sources are linked, never formally cited.** Erik (2026-09-02):
  ...
  so no reader takes it for one of ours (see [[0-numbers]] discipline).
